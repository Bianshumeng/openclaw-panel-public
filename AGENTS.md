...
### 7) 镜像发版流程（硬性）
- 发版入口统一走 GitHub Actions（不再依赖本地手工构建后上传）。
- 控制台镜像必须通过 `cd-panel-image` 工作流发布；Bot 镜像必须通过 `cd-openclaw-bot-image` 工作流发布。
- 发版前必须确认：
  - 控制台运行产物构建成功（`npm run build:runtime`）。
  - 最小验证通过（至少 `npm run test:unit`，建议全量 `npm run test`）。
- 发版后必须确认：
  - 两个工作流状态为 `success`。
  - GHCR 标签已生成（版本号、`latest`、`sha-*`）。
  - 更新页目标仓库地址与实际 GHCR 地址一致。

### 8) 自动化发版脚本与 CI/CD 维护（硬性）
- 默认使用脚本 `npm run release:images -- --target <panel|bot|all> [参数]` 触发发版，不再手敲零散命令。
- 脚本必须支持参数化版本（例如 `--panel-version`、`--bot-version`）与 Bot 源码来源（`--bot-repo`、`--bot-ref`）。
- CI/CD 工作流变更属于高敏变更：每次改动后必须做一次真实 workflow_dispatch 验证，并记录结果链接。
- 新增/修改发布相关脚本后，必须同步更新本文件对应约束，确保后续成员按同一流程执行。