# 2026-02-20_103_去Docker直装迁移记录

## 基本信息
- 目标机器：`vmid=103`（`vm-260219-0002`，`10.10.10.12`）
- 执行入口：母机 `9900K-1` 通过 QGA（`qm guest exec 103`）
- 执行日期：2026-02-20
...
## 备份与校验（已完成）
1. 精准备份目录：`/root/migrate-backup-103-2026-02-20_072927`
2. 备份包：
   - `data-openclaw.tgz`
   - `data-panel.tgz`
   - `opt-openclaw.tgz`
...
4. 校验结果：`sha256sum -c` 全部 `OK`，并完成 `tar -tzf` 可读性校验
...
4. 网关配置改造（对齐现有 Nginx）：
   ...
   - 配置备份：`/root/.openclaw/openclaw.json.bak.20260220073320`
5. 面板直装部署：
   - 发布版本：`v2026.02.20-r4`
...
7. 新建并启用系统服务：
   ...
   - 状态：`enabled + active`
8. 权限等级确认（root）：
   - `openclaw-gateway.service` 配置命中 `User=root`
   - `openclaw-panel.service` 配置命中 `User=root`
   - 主进程实测：
     - `openclaw-gateway`：`root`（PID `7530`）
     - `openclaw-panel`：`root`（PID `7584`）
...
4. 数据完整性抽样：
   - 会话文件数：`2 -> 2`
   - workspace 文件数：`4703 -> 4703`
5. Docker 保留状态（回滚兜底）：
   - `openclaw-panel`：`Exited (137)`
   - `openclaw-gateway`：`Exited (0)`

## 当前结论
1. 103 已完成去 Docker 化直装接管，核心链路可用。
...
2. 若配置损坏，可恢复：
   - `openclaw.json` 回滚到 `openclaw.json.bak.20260220073320`
   - 数据回滚到 `/root/migrate-backup-103-2026-02-20_072927/*`

## 下一步建议（待确认）
1. 你先进行 103 前端实测（隧道 + 页面配置读写 + Telegram 基础配置）。
2. 实测通过后执行 Docker 删除收尾（`docker compose down --remove-orphans`）。
3. 复用本记录流程继续迁移下一台存量小鸡。
