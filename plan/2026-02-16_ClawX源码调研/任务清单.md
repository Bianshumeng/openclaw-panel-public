...
## 6. Chat 能力迁移阶段（不替代龙虾 Bot 原生对话页）
- [ ] 6.1 新增 `对话控制台` 页面入口 | 验收：路由可访问，独立于原配置页
- [ ] 6.2 实现多会话管理（列表/切换/新建） | 验收：切换后历史正确，不串会话
- [ ] 6.3 实现流式消息展示 | 验收：delta 与 final 顺序稳定，消息不重复
- [ ] 6.4 实现思考状态展示开关 | 验收：开关仅影响显示，不影响后端请求
- [ ] 6.5 实现中止当前运行（Abort） | 验收：中止后发送态恢复且无残留锁定

## 7. 回归验证阶段
- [ ] 7.1 自动化测试（单元 + 回归）补齐 | 验收：`npm run test` 通过
- [ ] 7.2 手工回放关键路径 | 验收：模型、渠道、技能、日志、更新、对话控制台全链路通过
